...
SpotLight::SpotLight(const glm::vec3 color, float ambientIntensity,
                     ...
                     float quadratic)
    : SpotLight("Spot Light", color, ambientIntensity, diffuseIntensity,
                specularIntensity, position, direction, cutoff, outerCutoff,
                constant, linear, quadratic) {
}

SpotLight::SpotLight(std::string name, const glm::vec3 color,
    ...
    : BaseLight(
          name, LightType::SPOT, color, ambientIntensity,
...
void SpotLight::Bind(Shader& shader, int idx) {
    shader.SetUniform3f(fmt::format("u_spotLights[{}].position", idx),
                        m_position.x, m_position.y, m_position.z);
    shader.SetUniform3f(fmt::format("u_spotLights[{}].direction", idx),
                        m_direction.x, m_direction.y, m_direction.z);
    shader.SetUniform3f(fmt::format("u_spotLights[{}].color", idx), m_color.r,
                        m_color.g, m_color.b);
    shader.SetUniform1f(fmt::format("u_spotLights[{}].ambientIntensity", idx),
                        m_ambientIntensity);
    shader.SetUniform1f(fmt::format("u_spotLights[{}].diffuseIntensity", idx),
                        m_diffuseIntensity);
    ...
    shader.SetUniform1f(fmt::format("u_spotLights[{}].outerCutoff", idx),
                        m_outerCutoff);

    shader.SetUniform1f(fmt::format("u_spotLights[{}].constant", idx),
                        m_constant);
    shader.SetUniform1f(fmt::format("u_spotLights[{}].linear", idx), m_linear);
    shader.SetUniform1f(fmt::format("u_spotLights[{}].quadratic", idx),
                        m_quadratic);
}

void SpotLight::Unbind(Shader& shader, int idx) {
    shader.SetUniform3f(fmt::format("u_spotLights[{}].position", idx), 0, 0, 0);
    shader.SetUniform3f(fmt::format("u_spotLights[{}].direction", idx), 0, 0,
                        0);
